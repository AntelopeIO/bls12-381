...
void TestGroth16()
{
    ...
    if(!groth16::Proof::fromJacobianBytesBE(proof.toJacobianBytesBE()).value().equal(proof)) throw invalid_argument("Groth16, proof: JacobianBytesBE error");
    if(!groth16::Proof::fromJacobianBytesLE(proof.toJacobianBytesLE()).value().equal(proof)) throw invalid_argument("Groth16, proof: JacobianBytesLE error");
    if(!groth16::Proof::fromAffineBytesBE(proof.toAffineBytesBE()).value().equal(proof)) throw invalid_argument("Groth16, proof: AffineBytesBE error");
    if(!groth16::Proof::fromAffineBytesLE(proof.toAffineBytesLE()).value().equal(proof)) throw invalid_argument("Groth16, proof: AffineBytesLE error");
    if(!groth16::Proof::fromCompressedBytesBE(proof.toCompressedBytesBE()).value().equal(proof)) throw invalid_argument("Groth16, proof: CompressedBytesBE error");
    ...
    if(!groth16::VerifyingKey::fromJacobianBytesBE(vk.toJacobianBytesBE().data()).value().equal(vk)) throw invalid_argument("Groth16, vk: JacobianBytesBE error");
    if(!groth16::VerifyingKey::fromJacobianBytesLE(vk.toJacobianBytesLE().data()).value().equal(vk)) throw invalid_argument("Groth16, vk: JacobianBytesLE error");
    if(!groth16::VerifyingKey::fromAffineBytesBE(vk.toAffineBytesBE().data()).value().equal(vk)) throw invalid_argument("Groth16, vk: AffineBytesBE error");
    if(!groth16::VerifyingKey::fromAffineBytesLE(vk.toAffineBytesLE().data()).value().equal(vk)) throw invalid_argument("Groth16, vk: AffineBytesLE error");
    if(!groth16::VerifyingKey::fromCompressedBytesBE(vk.toCompressedBytesBE().data()).value().equal(vk)) throw invalid_argument("Groth16, vk: CompressedBytesBE error");

    groth16::PreparedVerifyingKey pvk = groth16::prepare_verifying_key(vk);
    if(!groth16::PreparedVerifyingKey::fromJacobianBytesBE(pvk.toJacobianBytesBE().data()).value().equal(pvk)) throw invalid_argument("Groth16, pvk: JacobianBytesBE error");
    if(!groth16::PreparedVerifyingKey::fromJacobianBytesLE(pvk.toJacobianBytesLE().data()).value().equal(pvk)) throw invalid_argument("Groth16, pvk: JacobianBytesLE error");
    if(!groth16::PreparedVerifyingKey::fromAffineBytesBE(pvk.toAffineBytesBE().data()).value().equal(pvk)) throw invalid_argument("Groth16, pvk: AffineBytesBE error");
    if(!groth16::PreparedVerifyingKey::fromAffineBytesLE(pvk.toAffineBytesLE().data()).value().equal(pvk)) throw invalid_argument("Groth16, pvk: AffineBytesLE error");
